...
class Vertex:
    ...
    def __init__(self, mesh, loop):
        ...
        try:
            self.uvs = tuple([mesh.uv_layers.active.data[loop_idx].uv.freeze()])
        except IndexError:
            print("IndexError:", mesh.name)
            self.uvs = tuple([Dummy_UV(0.0, 0.0)])
        except AttributeError:
            print("AttributeError:", mesh.name)
            self.uvs = tuple([Dummy_UV(0.0, 0.0)])
...
def export_meshes(state, meshes, exportable_meshes):
    def export_mesh(mesh):
        print("Trying to export mesh {}...".format(mesh.name))
        if mesh.name not in exportable_meshes:
            print("Mesh {} is not exportable".format(mesh.name))
            return None
...
#            print("Mesh {} has no vertices".format(me.name))
            return None
        ...
        if max_vert_index > 65535 or state['settings']["always_index_uint"]:
    ...
    exported_meshes = {}
    for mesh in meshes:
        print("Checking mesh {} with {} users:".format(mesh.name, mesh.users))
...
def export_nodes(state, objects, exportable_meshes):
    ...
    def export_node(obj):
        ...
        if obj.type == 'MESH':
            ...
            exportable_meshes.append(mesh.name)
            print("Going to export object {} with mesh {}".format(obj.name, mesh.name))
            print(repr(obj.data))
...
def export_gltf(scene_delta, settings=None):
    ...
    exportable_meshes = []
    exported_nodes = export_nodes(state, state['objects'], exportable_meshes)
    print("exportable_meshes: ", repr(exportable_meshes))
    print("mesh_list: ", repr(mesh_list))
    ...
    gltf = {
        ...
        'meshes': export_meshes(state, mesh_list, exportable_meshes),
